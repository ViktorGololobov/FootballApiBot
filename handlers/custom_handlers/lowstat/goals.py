...
from states.lowstat_states.goals_states import LowStatGoals, LowStatGoalsForAgainst
...
@bot.callback_query_handler(func=lambda call: True, state=LowStatGoals.all_goals)
def goals_show(call: CallbackQuery) -> None:
    ...
    if call.data == 'goals_for':
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_for, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех забито голов:', call.message.chat.id, call.message.message_id
        )
        ...
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_against, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех пропущено голов:', call.message.chat.id, call.message.message_id
        )
...
@bot.callback_query_handler(func=lambda call: True, state=LowStatGoals.home_goals)
def goals_show(call: CallbackQuery) -> None:
    """
    ...
    if call.data == 'goals_for':
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_for, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех забито голов дома:', call.message.chat.id, call.message.message_id
        )
        ...
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_against, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех пропущено голов дома:', call.message.chat.id, call.message.message_id
        )
...
@bot.callback_query_handler(func=lambda call: True, state=LowStatGoals.away_goals)
def goals_show(call: CallbackQuery) -> None:
    """
    ...
    if call.data == 'goals_for':
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_for, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех забито голов на выезде:', call.message.chat.id, call.message.message_id
        )
        ...
        bot.set_state(call.message.chat.id, LowStatGoalsForAgainst.goals_against, call.message.chat.id)
        bot.edit_message_text(
            f'Меньше всех пропущено голов на выезде:', call.message.chat.id, call.message.message_id
        )
...
def goals_stat(result: str, goals_param: str, api_endswith: str, parameters: Dict, api_method_type: str) -> Iterable[str]:
    """
