...
from states.custom_states.goals_states import CustomStatGoals
...
from keyboards.inline.custom_buttons.goals_buttons import for_against_buttons_gen
from keyboards.inline.custom_buttons.main_buttons import button_generator_for_back


@bot.callback_query_handler(func=lambda call: True, state=CustomStatMainState.goals)
def goals_callback(call: CallbackQuery) -> None:
    ...
    if call.data == 'all_info':
        bot.set_state(call.message.chat.id, CustomStatGoals.all_goals, call.message.chat.id)
        ...
        bot.set_state(call.message.chat.id, CustomStatGoals.home_goals, call.message.chat.id)
        bot.edit_message_text(
            f'Нужна информация по забитым или пропущенным?', call.message.chat.id,
            call.message.message_id, reply_markup=for_against_buttons_gen()
        )
    elif call.data == 'away':
        bot.set_state(call.message.chat.id, CustomStatGoals.away_goals, call.message.chat.id)
        bot.edit_message_text(
            f'Нужна информация по забитым или пропущенным?', call.message.chat.id,
            call.message.message_id, reply_markup=for_against_buttons_gen()
        )
