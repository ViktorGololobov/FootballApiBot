...
@bot.message_handler(commands=['highstat_champ'])
def button_generator(message: Message) -> None:
    """
    Хэндлер для обработки команды highstat_champ. Создает кнопки, и выводит панель сообщением пользователю.

    :param message: команда highstat_champ
    """

    bot.set_state(message.from_user.id, UserMainCommandInfo.highstat, message.chat.id)
